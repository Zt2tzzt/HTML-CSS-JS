...
;(function (global, factory) {
  ...
  jQuery.fn = jQuery.prototype = {
    ...
    jquery: version,
    // constructor#liujun
    constructor: jQuery, // 修正构造函数，jQuery().__proto__.constructor
  ...
  jQuery.extend({
    ...
    each: function (obj, callback) {
      ...
      if (isArrayLike(obj)) {
        length = obj.length
        for (; i < length; i++) {
          // 这里用call绑定了this的指向，this指向 DOM element
    ...
    merge: function (first, second) {
      var len = +second.length, // 前面的 + 号是将假值（不包含undefined）转成 0
        j = 0,
        i = first.length
    ...
    remove: function (owner, key) {
      ...
      if (key !== undefined) {
        ...
        if (Array.isArray(key)) {
          ...
          key = key in cache ? [key] : key.match(rnothtmlwhite) || [] // 转成数组
        }
  ...
  function on(elem, types, selector, data, fn, one) {
    ...
    return elem.each(function () {
      // each 中的 this 拿到的是 元素，而不是jQuery 对象
      ...
      while (t--) {
        ...
        if (!(handlers = events[type])) {
          ...
          if (!special.setup || special.setup.call(elem, data, namespaces, eventHandle) === false) {
            // addEventListener#liujun
            if (elem.addEventListener) {
              // elem是DOM 元素，不是jQuery对象
    ...
    css: function (name, value) {
      return access(
        ...
        function (elem, name, value) {
          ...
          return value !== undefined
            ? jQuery.style(elem, name, value) // set
            : jQuery.css(elem, name) // get
        },
        name,
        value,
        arguments.length > 1
  ...
  jQuery.fn.extend({
    ...
    // animate#liujun  属性  持续时间  缓动函数  结束回调
  ...
  if (typeof noGlobal === 'undefined') {
    // window#liujun 执行这个代码之前，前面的代码已对其它框架的 jQuery 和 $ 熟悉备份到 _jQuery 和 _$ 变量中
