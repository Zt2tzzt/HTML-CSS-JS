...
- 每一个元素，在标准流里面，都属于一个 FC；
- 块级元素的布局，属于块级格式化上下文（Block Formatting Context）（BFC）。
- 行内级元素的布局，属于行内级格式化上下文（Inline Formatting Context）（IFC）。
...
- 浮动元素的父元素，触发 BFC 形成独立的块级格式化上下文（Block Formatting Context）；
- 浮动元素的父元素，height 为 auto；
...
- 如果有块级元素，是由最顶层块盒子的上边缘，和最底层块盒子的下边缘之间的距离。
...
方式三：比较常用的是在 CSS 中通过 `@media` 来使用不同的 CSS 规则（掌握）；
