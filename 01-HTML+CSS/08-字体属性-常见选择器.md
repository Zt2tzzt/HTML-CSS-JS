...
- 前3个属性值可调换顺序，也可省略。之后的属性值不能调换顺序，line-height 可省略。
...
2. 目标伪类（target pseudo-classes） 当元素id，匹配当前url的fragment时，伪类生效。
	- :target
3. 语言伪类（language pseudo-classes） 如设置过 lang="en" 的元素，伪类 :lang(en) 生效（ lang 是html元素的全局属性）。
	- :lang( )
...
1. 除了a元素，:focus、:hover、:active也能用在其他元素上。
