...
计算机只能存储和处理 0、1 组成的二进制数字；它的底层硬件实现，是用电路的开、闭两个状态，表示 0 和 1 两个数字的。
...
形式二：内部样式表（internal style sheet），也称为：文档样式表（document style sheet）、内嵌样式表（embed style sheet）、内联样式表。
