...
IIFE，会创建一个独立的执行上下文环境（作用域），可以避免函数中对外界的访问，也避免了外界对函数内部变量的修改。常被用于：
...
- 对象类型，涉及到 JavaScript 的各个方面，所以掌握对象类型，非常重要；
- 对象类型是一种存储键值对（key-value）的更复杂的数据类型；

对象类型的作用：

- 基本数据类型可以存储一些简单的值，但是现实世界的事物抽象成程序时，往往比较复杂；
- 比如一个人，有自己的特性（比如：姓名、年龄、身高），有一些行为（比如：跑步、学习、工作）；
- 比如一辆车，有自己的特性（比如：颜色、重量、速度），有一些行为（比如：行驶）；
