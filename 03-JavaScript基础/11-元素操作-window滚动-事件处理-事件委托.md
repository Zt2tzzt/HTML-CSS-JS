...
创建元素不推荐的 2 个方法。有什么弊端。

- `document.write`，对于复杂的内容、元素关系拼接并不方便。早期没有 DOM 的时候使用的方案。
...
一般将 DOM 对象放在全局，而不是放在某个回调函数中，提高性能。实现计数器案例。
...
- `offsetLeft`：距离最近定位元素的 x
- `offsetTop`：距离最近定位元素的 y
...
2. DOM 属性（property），通过元素对象的 `onxxx` 属性（property）来监听事件，
...
事件监听默认事件流为事件冒泡，如何监听事件捕获？传入第 3 个布尔类型的参数为 `true`
