...
| 属性       | 描述                              |
| ---------- | --------------------------------- |
| onkeydown  | 发生在某个键盘按键按下后。        |
| onkeypress | 发生在文本被输入后。              |
| onkeyup    | 发生在键盘按键松开/文本输入完成。 |
...
| 属性     | 描述                                                     |
| -------- | -------------------------------------------------------- |
| onchange | 表单元素的内容改变时触发，内容确定改变时，如失去焦点时。 |
| oninput  | 元素获取用户输入时触发                                   |
| onfocus  | 元素获取焦点时触发                                       |
| onblur   | 元素失去焦点时触发                                       |
| onreset  | 表单重置时触发                                           |
...
<body>
  ...
  <script>
    ...
    // 2.底部titles的切换, 同时切换轮播图
    titleListEl.onmouseover = function(event) {
      // 1.1.确定发生鼠标进入的元素（a元素占据了整个li）
