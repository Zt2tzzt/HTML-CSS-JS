...
当我们通过引用对象的属性 key 来获取一个 value 时，它会触发 [[Get]] 的操作；这个操作，会首先检查该对象是否有对应的属性：

- 如果有的话，就使用它；

- 如果没有，那么会访问对象 [[prototype]] 内置属性指向的对象上的属性；

通过字面量直接创建一个对象，这个对象也会有这样的属性，只要是对象都会有这样的一个 [[prototype]] 内置属性；
...
构造函数，在通过 `new` 操作创建对象时，将这个显式原型，赋值给创建出来对象的隐式原型。

我们前面讲过 `new` 操作符和构造函数创建对象，会经理如下步骤：

1. 在内存中创建一个新的对象（空对象）；
2. 这个对象内部的 [[prototype]] 隐式原型，会被赋值为该构造函数的 `prototype` 显示原型；
...
函数的显示原型 `prototype` 上，有一个属性 `constructor` 属性，它指向当前的函数对象。
...
当需要在函数的原型对象上，添加过多的属性时，通常我们会重写整个原型对象。

- 为 `Foo` 构造函数的 `prototype` 赋值一个新对象时，这个新对象的 `constructor` 属性，会指向 `Object` 构造函数，而不是 `Foo` 构造函数。

如果希望 `constructor` 属性，指向 `Person` 构造函数，需要进行手动添加，如下方代码所示：
...
Object.defineProperty(Foo.prototype, 'constructor', {
  configurable: true,
  writable: true,
  // 当使用 defineProperty 方法设置属性时，enumerable 默认为 false
  value: Foo
})
```
...
- **封装**：可以将属性和方法封装到一个类（构造函数）中，这个过程称之为封装；
...
我们可以为对象的隐式原型赋值一个新对象，来组成原型链，如下方代码所示：
...
**借用构造函数（constructor stealing）**也称之为**经典继承**或者**伪造对象**，用它来实现继承，代码如下：
...
Student.prototype = Person.prototype  // 该行代码不能调换顺序。
...
写法二（有缺陷）：创建一个父类的实例对象 `new Person()`，用这个实例对象来作为子类的原型对象
...
将**借用构造函数（constructor stealing）继承**和**原型链继承**组合起来实现继承，称为**组合借用继承**。它存在的问题？
...
- 一份在子类的实例对象里面（也就是 Student 本身的）；
- 另一份在子类的实例对象对应的原型对象中（也就是 `stu.__proto__` 里面）；
...
寄生式继承函数，是由道格拉斯提出的，它是与原型式继承紧密相连的思想。它是结合原型式继承和工厂模式的一种实现方式。
...
现在，寄生式继承可以解决**原项链继承**的问题，将**寄生式继承**结合**借用构造函数继承**，实现**寄生组合式继承**
