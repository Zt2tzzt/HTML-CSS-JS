...
<script>
	function deepCopy(originValue) {
		...
		const isArr = Array.isArray(originValue)
		const newObj = isArr ? []: {} // 2.如果是对象类型, 才需要创建对象
		if (isArr) {
...
<script>
    ...
    if (typeof originValue === "function") { // 3.如果是函数function类型, 不需要进行深拷贝
		...
		const symbolKeys = Object.getOwnPropertySymbols(originValue) // 单独遍历symbol
		symbolKeys.forEach(skey => {
...
<script>
    ...
    if (map.get(originValue)) { // 4.如果是对象类型, 才需要创建对象
